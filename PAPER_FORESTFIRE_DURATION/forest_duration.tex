...
was converted into minutes. Subsequently, three distinct values were
created depending on the logarithmic value of the duration in minutes
of the forest fire. This value will be used as the target value in
...
algorithm.
\item The column MLP denotes the application of a neural network with $H=$
processing nodes. The network was trained with the method $ $ of
the WEKA.
\end{enumerate}
\begin{table}[H]
...
\textbf{1) Discover important features for predicting duration}

\textbf{2) create rules for determination of categories}

\textbf{3) using meteorological data }

4) more machine learning methods
