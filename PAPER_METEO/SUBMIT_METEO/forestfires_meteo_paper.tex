...
impacts, propagation dynamics, symmetrical or asymmetrical patterns
...
personnel in the field. Beyond that, examination of symmetrical or
asymmetrical patterns in fires, helps to understand the causes and
dynamics of their spread. The necessity of leveraging machine learning
...
rising temperatures, asymmetrical patterns, and the increasing frequency
...
and publicly available symmetrical datasets provided by the Hellenic
Fire Service for the years 2014--2021, which we further enriched
with meteorological data, corresponding to the prevailing conditions
at both the onset, and the suppression of each wildfire event. The
research concluded that the Feature Construction technique, using
Grammatical Evolution, combines both symmetrical and asymmetrical
conditions, that weather phenomena may provide and outperform other
methods in terms of stability and accuracy. Therefore, the asymmetric
phenomenon in our research is defined as the unpredictable outcome
of climate change (meteorological data) which prolongs the duration
of forest fires over time. Specifically, the model accuracy of wildfire
duration using Feature Construction, the mean error was 8.25\%, indicating
an overall accuracy of 91.75\%.}
...
Earth's ecosystems and human societies. Beyond that, according to
the Chair of ISO/TC 92 fire safety, Mr. P. Van Hees: ‘\emph{With losses
...
Climate change interacts synergistically with wildfires by increasing
drought, high temperatures, low humidity, lightning,\textbf{ }asymmetrical
patterns, and strong winds, leading to more severe and prolonged fire
seasons. Conversely, wildfires contribute to reinforcing climate change
\citep{fire5}. Thus, wildfires (along with the extraction and burning
of fossil fuels, and volcanic eruptions) mutually enhance Climate
Change, by further releasing carbon dioxide into the atmosphere \citep{fire_nasa}.
Concerning this, the Mediterranean region is recognized as a key \textquotedbl hot-spot\textquotedbl{}
...
on wildfire\textbf{ }asymmetrical patterns is highlighted as crucial
for protecting both the environment and public health in Greece \citep{fire_iliopoulos}.
...
calling on governments “to radically shift their investments in wildfires
to focus on prevention and preparedness” \citep{fire5}. On that ground,
despite the challenging conditions posed by Climate Change, driven
by endless human expansion and technological progress, we attempt
to transform this disadvantage into an advantage and asymmetrical
to symmetrical\textbf{,} by focusing our efforts on rising technology
itself. That is to say, artificial intelligence, particularly machine
learning, has emerged as a helpful ally in addressing this global
issue, offering innovative solutions and aiding sustainable development
\citep{ai_climate,ai_ml}. 

Machine learning, applied to a collection of techniques and algorithms
that make it easier for systems to identify symmetrical / asymmetrical\textbf{
}patterns and make decisions based on data enriching their performance
over time without explicitly being programmed for specific tasks \citep{iso_ml}.
This was the vision of Alan Turing, when, in 1936, he wrote his PhD
‘On Computable Numbers, with an application to the Entscheidung problem’
\citep{ml_turing}. That is to say, machine learning, is a vital branch
of artificial intelligence, presents golden opportunities for businesses
and society alike. Beyond its countless advantages, it plays a critical
role in driving innovative advancements in climate change, adaptation,
and mitigation. By accelerating the development of resolutions to
some of the most urgent asymmetrical challenges facing the planet,
machine learning is transforming the process we addressing global
environmental issues \citep{iso_ml}. Even though modeling multiplex
environmental variables repeatedly presents challenges on account
of the significant computation resources required and the diverseness
or complexity of data formats \citep{ml_review}. Machine learning
algorithms, nevertheless, can bypass these asymmetrical challenges
...
helpful when dealing with frameworks involving numerous parameters
with complex asymmetrical physical properties, such as in forest fires.
Therefore, adopting a machine learning technique to fire management
...
\begin{figure}[H]
\begin{centering}
\includegraphics{fire_algorithms}
\par\end{centering}
...
Naïve Bayes, has also been employed to face fire-related challenges
...
Using the Naive Bayes Algorithm'' \citep{fire_naive2}. Karo, presented
...
In addition, Random Forests have been widely employed in forest fire
...
\citep{fire_rf2}. Song, demonstrated the superiority of the Random
Forest model, over SVM, XGBoost, and LightGBM, in predicting forest
lightning fires ``Interpretable artificial intelligence models for
predicting lightning prone to inducing forest fires'' \citep{Song}.
As well, Gao conducted a forest fire risk prediction study in China,
``Forest-fire-risk prediction based on random forest and back propagation
neural network of Heihe area in Heilongjiang province, China'' \citep{fire_rf3}.
Hu, developed and validated results related to fire events in fuel
...
achieving 90.9\% \citep{duration_wang}. Subsequently, researchers
also highlighted the LSTM method, in a study concerning fires in enclosed
...
asymmetrical climate change are commonly ``inflamed'' highlighting
...
On this subject, regarding the asymmetric effects and phenomena brought
about by Climate Change, we will indicatively refer to certain studies.
In 2001, Flato \& Boer introduced the paper `` Warming asymmetry
in Climate Change simulations'' \citep{Flato=000020and=000020Boer}.
...
In 2012, Xu \& Ramanathan, bring out: ``Latitudinally asymmetric
response of global surface temperature: Implications for regional
climate change'' \citep{Xu=000020and=000020Ramanathan}. In 2018,
Shunchuan printed: ``A symmetrical CO2 peak and asymmetrical climate
change during the middle Miocene'' \citep{Shunchuan2018}. In 2021,
Gao issued ``Asymmetrical lightning fire season expansion in the
boreal forest of Northeast China'' \citep{Gao2021}.
...
existing symmetrical and asymmetrical data attributes \citep{genfc1}.
...
were applied to data collected for the Greek case that contained weather
information.

To main contributions of the current work can be summarized to the
following highlights:
\begin{enumerate}
\item The incorporation of Forest Fires Greek data from the years 2014 -
2021 (57.904 entries).
\item The inclusion of meteorological data for each fire event.
\item The classification of Forest Fires according to their Duration.
\item The usage of Feature Construction Techniques.
\item The prediction of the Forest Fire Duration.
\item Main contributions: Estimating Fire Duration and Analyzing the Interaction
Between Environmental Conditions and Fire Duration.
\end{enumerate}
...
and selection techniques that will be applied to the datasets incorporated
...
and covering all involved entities. Regarding burned areas, the dataset
included measurements for the following categories: forests, forested
areas, groves, grasslands, reed beds and wetlands, agricultural lands,
crop residues, and landfills. 
...
well as contracted helicopters and aircraft. From the raw data obtained
from the Hellenic Fire Service, we removed all records that lacked
essential values such as the date, fire ignition time, or fire extinguishing
time, as it would not be possible to proceed with predicting the duration
of a fire without these fundamental parameters. This data preprocessing
step ensured the retention of complete and consistent information,
establishing a symmetric and reliable basis for subsequent analyses,
while mitigating potential asymmetric data gaps. The original number
of patterns for every year and the deleted patterns (due to missing
data) are presented in Table \ref{tab:The-original-datasets}.

\begin{table}[H]
\caption{The original size of datasets and the eliminated rows.\protect\label{tab:The-original-datasets}}

\centering{}%
\begin{tabular}{|c|c|c|c|}
\hline 
YEAR & RAW DATA & DELETED & FINAL DATA\tabularnewline
\hline 
\hline 
2014 & 6834 & 1158 & 5676\tabularnewline
\hline 
2015 & 8117 & 1358 & 6759\tabularnewline
\hline 
2016 & 10258 & 1642 & 8616\tabularnewline
\hline 
2017 & 10355 & 1678 & 8677\tabularnewline
\hline 
2018 & 8005 & 1363 & 6642\tabularnewline
\hline 
2019 & 9499 & 2280 & 7219\tabularnewline
\hline 
2020 & 11798 & 4579 & 7096\tabularnewline
\hline 
2021 & 9513 & 2417 & 7096\tabularnewline
\hline 
\textbf{TOTAL} & \textbf{74379} & \textbf{16475} & \textbf{57904}\tabularnewline
\hline 
\end{tabular}
\end{table}
...
phases of the fires. In this context, incorporating meteorological
variables from both the ignition and extinguishment periods captures
the asymmetric evolution of weather conditions throughout the fire’s
duration. Forest fires in Greece often evolve under rapidly changing
meteorological patterns, where factors such as wind speed, temperature,
and humidity fluctuate significantly between ignition and suppression.
Therefore, integrating both sets of meteorological data allows for
a more comprehensive and symmetric representation of the fire event,
enhancing the model’s ability to approximate real world dynamics.
However, the primary aim of this study is not real - time operational
prediction, but rather the post-event modelling and understanding
of the duration of forest fires under realistic and dynamic environmental
conditions.
...
long duration fire.
\end{enumerate}
The specific temporal categorization was determined based on the distribution
of forest fire duration data across the Greek territory, as observed
in the dataset we processed. This data-driven approach ensured that
the selected cut-points ($\le$360 min, 361--7200 min, $>$7201 min)
accurately reflected the natural groupings present within the observed
fire duration patterns. It should be noted that in Greek territory,
the combination of fuel types (such as pine and fir forests), geomorphological
features (including mountainous terrain, ravines, and steep slopes),
and climatic conditions (temperature, humidity, and wind speed) are
particularly distinctive. As a result, if a wildfire is not brought
under control within the first few hours, it tends to spread rapidly
beyond containment, potentially burning for several days.
...
The primary objective of PCA, is to reduce the dimensionality of a
dataset, while retaining the most significant patterns, and relationships,
...
face recognition, economic and environmental sciences, medical prediction,
...
of the method when working with finite or limited data, a common scenario,
...
genetic improvement. Noteworthy, is the fact that the 64.29\%, of
the total variance explained, can be considered relatively low. This
suggests that a significant amount of unexplained information remains
which is not captured by the four principal components. Moreover,
Gambardella et al. used the PCA technique for monitoring the cultivation
of cannabis in Albania. Specifically, with PCA they remove redundant
spectral information from multiband datasets \citep{pca_parente}.
...
between IAQ and its influencing factors. In particular, indoor air
...
Vector Machine (SVM) \citep{svm}. They create the Breast Cancer Risk
...
as using an f-test between the target and the features. When two features
are similar, MRMR prioritizes only the one with the highest relevance.
...
machine learning (EML) \citep{eml}. The most impactful features are
identified by performing MRMR feature selection analysis on the extracted
deep features \citep{mrmr_eristi}.
...
%
\end{enumerate}
The diagram \ref{fig:The-full-pipeline} depicts the proposed FC scheme,
from raw data to statistically substantiated results. Incident records
from the Hellenic Fire Service (2014--2021) are combined with meteorological
data from OpenWeather at ignition and extinguishment times. Each incident
is geocoded with OpenCage (latitude/longitude) and merged with the
corresponding weather measurements, yielding a consistent spatiotemporal
profile per event. The unified dataset is then cleaned and normalized
(handling missing values, harmonizing units and timing, and applying
feature scaling). The target variable is defined as fire duration
in minutes and categorized into Short, Medium, and Long according
to the thresholds specified in the manuscript. Feature Construction
is performed by the proposed GE-based FC (QFc), which searches for
nonlinear expressions of the original variables and produces a compact
set of constructed features $\left(N_{f}\right)$ tailored to the
classification task. These features feed the final neural-network
classifier, which is trained and evaluated using classification error
(\%) as the primary metric.
\begin{figure}[H]
\includegraphics[scale=0.75]{FC_Pipeline}

\caption{The full pipeline of the used process.\protect\label{fig:The-full-pipeline}}
...
environment of Optimus \citep{optimus}, that can be downloaded from
\url{https://github.com/itsoulos/GlobalOptimus.git}(accessed on 11
October 2025). as well as the WEKA programming tool \citep{weka_main}.\textbf{
}The WEKA software has been incorporated in a series of problems \citep{weka_education1,weka_education2,weka_medical1,weka_medical2}.
Each experiment was conducted 30 times and the average classification
error is reported, using different seed for the random generator each
time and the ten - fold cross validation procedure was used to validate
the experimental results. The values of parameters for the used methods
are shown in Table \ref{tab:values}. 

\begin{table}[H]
\caption{The values for each parameter of the proposed method.\protect\label{tab:values}}
...
\hline 
$B_{I}$ & Iterations for BFGS & 2000\tabularnewline
\hline 
$A_{I}$ & Iterations for ADAM & 2000\tabularnewline
\hline 
$\beta_{1}$ & Parameter for ADAM & 0.9\tabularnewline
\hline 
$\beta_{2}$ & Parameter for ADAM & 0.999\tabularnewline
\hline 
\end{tabular}
\end{table}
...
\item The column BAYESNN presents results using the Bayesian optimizer from
the open-source BayesOpt library \citep{bayesopt}. This method was
used to train a neural network with 10 processing nodes.
\item The column DNN denotes the usage of a deep neural network as implemented
in the Tiny Dnn library, that can be downloaded freely from \url{https://github.com/tiny-dnn/tiny-dnn}(accessed
on 10 October 2025). The optimization method AdaGrad \citep{adagrad}
was used to train the neural network in this case.
...
\hline 
{\footnotesize YEAR} & {\footnotesize BAYES} & {\footnotesize ADAM} & {\footnotesize BFGS} & {\footnotesize MRMR} & {\footnotesize PCA} & {\footnotesize BAYESNN} & {\footnotesize DNN} & {\footnotesize NNC} & {\footnotesize FC}\tabularnewline
\hline 
\hline 
{\footnotesize 2014} & {\footnotesize 11.41\%} & {\footnotesize 13.00\%} & {\footnotesize 12.38\%} & {\footnotesize 9.68\%} & {\footnotesize 15.50\%} & {\footnotesize 12.14\%} & {\footnotesize 13.01\%} & {\footnotesize 9.21\%} & {\footnotesize 8.04\%}\tabularnewline
\hline 
{\footnotesize 2015} & {\footnotesize 10.49\%} & {\footnotesize 11.94\%} & {\footnotesize 11.25\%} & {\footnotesize 8.49\%} & {\footnotesize 15.03\%} & {\footnotesize 11.48\%} & {\footnotesize 11.89\%} & {\footnotesize 9.17\%} & {\footnotesize 7.51\%}\tabularnewline
\hline 
{\footnotesize 2016} & {\footnotesize 10.79\%} & {\footnotesize 12.95\%} & {\footnotesize 11.88\%} & {\footnotesize 9.45\%} & {\footnotesize 12.93\%} & {\footnotesize 12.53\%} & {\footnotesize 12.89\%} & {\footnotesize 10.12\%} & {\footnotesize 8.60\%}\tabularnewline
\hline 
{\footnotesize 2017} & {\footnotesize 53.36\%} & {\footnotesize 12.68\%} & {\footnotesize 12.65\%} & {\footnotesize 12.65\%} & {\footnotesize 12.64\%} & {\footnotesize 12.65\%} & {\footnotesize 12.62\%} & {\footnotesize 12.61\%} & {\footnotesize 12.66\%}\tabularnewline
\hline 
{\footnotesize 2018} & {\footnotesize 9.39\%} & {\footnotesize 10.48\%} & {\footnotesize 14.97\%} & {\footnotesize 9.21\%} & {\footnotesize 10.49\%} & {\footnotesize 10.02\%} & {\footnotesize 10.38\%} & {\footnotesize 9.29\%} & {\footnotesize 7.72\%}\tabularnewline
\hline 
{\footnotesize 2019} & {\footnotesize 7.79\%} & {\footnotesize 9.44\%} & {\footnotesize 9.66\%} & {\footnotesize 8.39\%} & {\footnotesize 9.72\%} & {\footnotesize 8.94\%} & {\footnotesize 9.41\%} & {\footnotesize 7.03\%} & {\footnotesize 6.62\%}\tabularnewline
\hline 
{\footnotesize 2020} & {\footnotesize 40.26\%} & {\footnotesize 9.56\%} & {\footnotesize 9.80\%} & {\footnotesize 9.55\%} & {\footnotesize 9.76\%} & {\footnotesize 9.50\%} & {\footnotesize 9.50\%} & {\footnotesize 9.50\%} & {\footnotesize 9.61\%}\tabularnewline
\hline 
{\footnotesize 2021} & {\footnotesize 11.81\%} & {\footnotesize 11.06\%} & {\footnotesize 12.90\%} & {\footnotesize 10.57\%} & {\footnotesize 11.03\%} & {\footnotesize 10.90\%} & {\footnotesize 10.62\%} & {\footnotesize 10.80\%} & {\footnotesize 9.55\%}\tabularnewline
\hline 
{\footnotesize\textbf{AVERAGE}} & {\footnotesize\textbf{18.88\%}} & {\footnotesize\textbf{10.88\%}} & {\footnotesize\textbf{11.25\%}} & {\footnotesize\textbf{9.25\%}} & {\footnotesize\textbf{11.63\%}} & {\footnotesize\textbf{10.50\%}} & {\footnotesize\textbf{10.75\%}} & {\footnotesize\textbf{9.38\%}} & {\footnotesize\textbf{8.25\%}}\tabularnewline
\hline 
\end{tabular}}{\footnotesize\par}
\end{table}
Also the standard deviation for the experimental results is depicted
in Table \ref{tab:std}.
\begin{table}[H]
\caption{Standard deviation values for all used methods.\protect\label{tab:std}}

\centering{}{\footnotesize{}%
\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
\hline 
{\footnotesize YEAR} & {\footnotesize ADAM} & {\footnotesize BFGS} & {\footnotesize MRMR} & {\footnotesize PCA} & BAYESNN & DNN & {\footnotesize NNC} & {\footnotesize FC}\tabularnewline
\hline 
\hline 
{\footnotesize 2014} & 0.06 & 0.47 & 0.07 & 0.10 & 0.33 & 0.06 & 0.99 & 0.17\tabularnewline
\hline 
{\footnotesize 2015} & 0.08 & 0.19 & 0.12 & 0.06 & 0.22 & 0.11 & 0.70 & 0.17\tabularnewline
\hline 
{\footnotesize 2016} & 0.04 & 0.12 & 0.07 & 0.03 & 0.20 & 0.06 & 0.57 & 0.19\tabularnewline
\hline 
{\footnotesize 2017} & 0.05 & 0.03 & 0.06 & 0.14 & 0.05 & 0.04 & 0.11 & 0.04\tabularnewline
\hline 
{\footnotesize 2018} & 0.06 & 0.51 & 0.16 & 0.05 & 0.20 & 0.08 & 0.47 & 0.17\tabularnewline
\hline 
{\footnotesize 2019} & 0.07 & 0.45 & 0.12 & 0.03 & 0.29 & 0.05 & 0.49 & 0.18\tabularnewline
\hline 
{\footnotesize 2020} & 0.05 & 0.07 & 0.03 & 0.04 & 0.04 & 0.04 & 0.12 & 0.19\tabularnewline
\hline 
{\footnotesize 2021} & 0.08 & 0.12 & 0.14 & 0.13 & 0.26 & 0.08 & 0.48 & 0.18\tabularnewline
\hline 
\end{tabular}}{\footnotesize\par}
\end{table}
Year-wise results show that the proposed FC method achieves the lowest
average classification error (8.25\%), outperforming NNC (9.38\%),
MRMR (9.25\%), BAYESNN (10.50\%), DNN (10.75\%), ADAM (10.88\%), BFGS
(11.25\%), and PCA (11.63\%) (Table \ref{tab:expr}). FC attains the
best annual score in 6 out of 8 years (2014, 2015, 2016, 2018, 2019,
2021), with two exceptions: in 2017 NNC is marginally better (12.61\%
vs. 12.66\%), and in 2020 other methods tie at the lowest error (BAYESNN/DNN/NNC
= 9.50\% vs. 9.61\% for FC). Notable anomalies appear for BAYES in
2017 (53.36\%) and 2020 (40.26\%), suggesting that this classifier
is sensitive to the data characteristics in those years. Regarding
stability, standard deviations indicate high variability for NNC (e.g.,
0.99 in 2014) and elevated dispersion for BFGS in some years (up to
0.51), whereas FC maintains moderate and consistent variability (\ensuremath{\approx}0.04--0.19),
offering a robust accuracy-reliability balance. Overall, the evidence
supports FC as the top performer with consistently lower errors especially
in recent years apart from specific edge cases in 2017 and 2020. 

In Figure \ref{fig:Average-execution-time} the average execution
time for all methods participated in the experiments is outlined. 

\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.75]{forest_meteo_time}
\par\end{centering}
\caption{Average execution time for all methods incorporated in the conducted
experiments.\protect\label{fig:Average-execution-time}}

\end{figure}
As expected, the proposed method requires significantly more time
than other techniques since it consists of a Genetic Algorithm that
in each iteration minimizes the error in a large series of RBF networks,
in order to calculate the corresponding fitness values.

Using paired Wilcoxon signed-rank tests across the classification
datasets, the proposed FC method exhibits a consistent statistical
advantage over all competing models. The pairwise comparisons FC vs
BAYES (p=0.016), ADAM (p=0.023), BFGS (p=0.016), MRMR (p=0.039), PCA
(p=0.016), BAYESNN (p=0.039), DNN (p=0.039), and NNC (p=0.039) are
all significant at $\alpha=0.05$. None of the p-values falls below
0.01, therefore the evidence corresponds to “significant” rather than
“highly” or “extremely” significant according to the Figure \ref{fig:statExper}
legend. The uniform pattern of p\textless 0.05 across all pairs substantiates
that FC attains systematically lower error than every alternative
model on the evaluated datasets, with the strongest statistical indication
against BAYES, BFGS, and PCA (p=0.016). 
...
an average error of 8.25\%. The error rates range from 6.62\% in 2019,
...
further improvement, with an average error of 8.13\%, which is the
...
from 9.03\% for $N_{f}=1$ to 8.50\% for $N_{f}=3$ demonstrates that
...
\hline 
\textbf{AVERAGE} & \textbf{8.50\%} & \textbf{8.25\%} & \textbf{8.13\%}\tabularnewline
\hline 
\end{tabular}
\end{table}
...
Furthermore, to study the accuracy of the measurements in relation
to the number of chromosomes ($N_{c})$, another experiment was conducted
using only the techniques based on Grammatical Evolution and with
a variable number of chromosomes, and the results are presented in
Table \ref{tab:experNG}.

\begin{table}[H]
\caption{The effect of the number of chromosomes $N_{c}$ to the accuracy of
methods based on Grammatical Evolution.\protect\label{tab:experNG}}

\centering{}%
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline 
 & \multicolumn{3}{c|}{NNC} & \multicolumn{3}{c|}{FC}\tabularnewline
\hline 
\hline 
YEAR & $N_{c}=100$ & $N_{c}=200$ & $N_{c}=500$ & $N_{c}=100$ & $N_{c}=200$ & $N_{c}=500$\tabularnewline
\hline 
2014 & 9.90\% & 9.78\% & 9.21\% & 8.74\% & 8.77\% & 8.04\%\tabularnewline
\hline 
2015 & 9.13\% & 9.07\% & 9.17\% & 8.24\% & 8.30\% & 7.51\%\tabularnewline
\hline 
2016 & 9.98\% & 10.18\% & 10.12\% & 8.98\% & 8.92\% & 8.60\%\tabularnewline
\hline 
2017 & 12.63\% & 12.63\% & 12.61\% & 12.66\% & 12.71\% & 12.66\%\tabularnewline
\hline 
2018 & 8.58\% & 8.80\% & 9.29\% & 7.96\% & 7.85\% & 7.72\%\tabularnewline
\hline 
2019 & 7.40\% & 7.48\% & 7.03\% & 6.55\% & 6.69\% & 6.62\%\tabularnewline
\hline 
2020 & 9.43\% & 9.51\% & 9.50\% & 9.50\% & 9.50\% & 9.61\%\tabularnewline
\hline 
2021 & 10.97\% & 10.98\% & 10.80\% & 9.65\% & 9.90\% & 9.55\%\tabularnewline
\hline 
AVERAGE & \textbf{9.13\%} & \textbf{9.25\%} & \textbf{9.38\%} & \textbf{8.38\%} & \textbf{8.39\%} & \textbf{8.25\%}\tabularnewline
\hline 
\end{tabular}
\end{table}
Studying this table, one finds that there are no large deviations
in the average classification error regardless of the number of chromosomes
