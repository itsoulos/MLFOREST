...
interpretations across different civilizations. In ancient Greece,
for example, seismic activity was attributed to the giant Enceladus,
who, according to myth, was buried beneath the earth by the gods and
would cause tremors as he struggled to break free from his subterranean
prison. Similarly, in Norse mythology, earthquakes were believed to
be the result of the imprisoned god Loki, who writhed in pain beneath
the ground, shaking the earth as a consequence of his suffering. Other
cultural traditions also developed unique cosmologies to explain seismic
events. It was not until the 18th and 19th centuries that a more positivist
and scientific approach began to emerge regarding the explanation
of earthquakes, recognizing their origin as stemming from processes
occurring beneath the Earth’s surface. In the present century, the
question of how earthquakes occur has been resolved, thanks to advancements
in scientific, geological, and geophysical research. It is now well
understood that seismic events result from the collision and movement
of lithospheric or tectonic plates. The contemporary challenge that
emerges, however, lies in whether such seismic phenomena can be accurately
predicted. In this paper, a systematic attempt is made to use techniques
based on Grammatical Evolution to determine the magnitude of earthquakes.
These techniques use freely available data in which the history of
large earthquakes is introduced before the application of the proposed
techniques. From the execution of the experiments, it became clear
that the use of these techniques can estimate the magnitude of earthquakes
more effectively compared to other machine learning techniques from
...
Earthquake Browser \url{http://ds.iris.edu/ieb/}. The NSF SAGE Facility
has been certified as a trustworthy data repository by the CoreTrustSeal
Standards and Certification Board. The data were collected for the
years 2004, 2010, and 2012, with each year comprising over 100,000
recorded earthquake events. For every recorded earthquake event, comprehensive
data were systematically gathered, including the date, exact time
of occurrence, geographic coordinates,depth, and magnitude. This information
enabled temporal analyses and the identification of seismic patterns
across different time intervals. Moreover, our data set employs the
Moment Magnitude scale, as it operates effectively across a broader
range of earthquake sizes, and is applicable on a global scale. During
the initial stages of preprocessing, it became evident that earthquake
...
model, within which 126,972 seismic events were recorded. Similarly,
in 2010, 635 regions were defined and encoded as input features, corresponding
to 294,432 earthquakes, while in 2012, 690 regions were established
as categorical input variables, with a total of 370,582 recorded seismic
...
operations as in the algorithm of subsection \ref{subsec:The-rule-production}.
\item \textbf{Termination check step}.
\begin{enumerate}
\item \textbf{Set} $k=k+1$
...
\hline 
\textbf{YEAR} & \textbf{$D_{c}$} & \textbf{MLP(BP)} & \textbf{MLP(RPROP)} & \textbf{MLP(BFGS)} & \textbf{RULE} & \textbf{NNC} & \textbf{FC}\tabularnewline
\hline 
\hline 
2004 & 10 & 0.44 & 0.24 & 0.82 & 0.16 & 0.16 & 0.17\tabularnewline
\hline 
2004 & 25 & 0.42 & 0.24 & 0.98 & 0.17 & 0.17 & 0.17\tabularnewline
\hline 
2004 & 50 & 0.43 & 0.24 & 0.87 & 0.17 & 0.17 & 0.16\tabularnewline
\hline 
2004 & 100 & 0.35 & 0.22 & 0.69 & 0.16 & 0.16 & 0.16\tabularnewline
\hline 
2004 & 500 & 0.45 & 0.27 & 0.65 & 0.16 & 0.16 & 0.16\tabularnewline
\hline 
2010 & 10 & 0.36 & 0.24 & 0.74 & 0.19 & 0.17 & 0.19\tabularnewline
\hline 
2010 & 25 & 0.37 & 0.21 & 0.49 & 0.19 & 0.18 & 0.17\tabularnewline
\hline 
2010 & 50 & 0.39 & 0.24 & 0.60 & 0.18 & 0.17 & 0.18\tabularnewline
\hline 
2010 & 100 & 0.31 & 0.27 & 0.40 & 0.19 & 0.18 & 0.19\tabularnewline
\hline 
2010 & 500 & 0.40 & 0.32 & 0.51 & 0.19 & 0.18 & 0.18\tabularnewline
\hline 
2012 & 10 & 0.33 & 0.22 & 0.45 & 0.18 & 0.17 & 0.19\tabularnewline
\hline 
2012 & 25 & 0.33 & 0.24 & 0.75 & 0.17 & 0.17 & 0.16\tabularnewline
\hline 
2012 & 50 & 0.36 & 0.23 & 0.21 & 0.17 & 0.17 & 0.16\tabularnewline
\hline 
2012 & 100 & 0.38 & 0.21 & 0.57 & 0.17 & 0.17 & 0.16\tabularnewline
\hline 
2012 & 500 & 0.35 & 0.29 & 0.85 & 0.17 & 0.17 & 0.16\tabularnewline
\hline 
\textbf{AVERAGE} &  & \textbf{0.378} & \textbf{0.245} & \textbf{0.639} & \textbf{0.175} & \textbf{0.170} & \textbf{0.171}\tabularnewline
\hline 
\end{tabular}
\end{table}

Here follows the evaluation of the performance of different machine
learning models for predicting earthquake magnitudes, based on data
from neighboring earthquakes. The Table \ref{tab:expResults} presents
regression errors for three years (2004, 2010, 2012) and five different
critical distances , comparing the performance of three models without
grammatical evolution and three models with grammatical evolution.
The models without grammatical evolution, specifically MLP(BP), MLP(RPROP),
and MLP(BFGS), exhibit higher errors compared to models using grammatical
evolution. The MLP(BFGS) model shows the largest error, with values
reaching up to 0.98 for a distance of 25 miles in 2004, indicating
instability under certain conditions. In contrast, MLP(RPROP) displays
the smallest error among models without grammatical evolution, with
an average value of 0.245, suggesting better adaptability. The models
with grammatical evolution RULE, NNC, and FC show significantly smaller
and more stable errors, with average values of 0.175, 0.17, and 0.171,
respectively. This stability suggests that grammatical evolution provides
robustness against data variations. The influence of distance appears
significant for models without grammatical evolution, particularly
for MLP(BFGS), where the error fluctuates between 0.21 and 0.98 depending
on distance. Conversely, in models with grammatical evolution, the
error remains nearly unchanged regardless of distance, highlighting
their ability to handle different spatial conditions. From a temporal
perspective, models without grammatical evolution show a trend of
improvement over time, with MLP(BFGS) reducing its average error from
0.82 in 2004 to 0.57 in 2012. However, this improvement is insufficient
to match the performance of models with grammatical evolution, which
maintain consistent performance across all years. In summary, the
results demonstrate that the use of grammatical evolution leads to
significantly better predictions, with smaller and more stable errors.
This stability is observed across different distances and years, suggesting
that models with grammatical evolution are less sensitive to data
variations. The superiority of these models may stem from their ability
to better recognize and exploit structures and relationships within
the data. Future research could focus on investigating the precise
mechanisms through which grammatical evolution enhances performance,
as well as comparisons with other advanced machine learning techniques.

Figure \ref{fig:2004} presents the regression errors for earthquake
magnitude prediction based on 2004 data. Significant differences are
observed between the two model categories. Models without grammatical
evolution (MLP(BP), MLP(RPROP), MLP(BFGS)) show higher errors compared
to models using grammatical evolution (RULE, NNC, FC). MLP(BFGS) exhibits
the largest errors, reaching 0.98 for 25-mile distances, indicating
instability under specific conditions. Conversely, MLP(RPROP) shows
the smallest errors among non-grammatical evolution models, with consistent
values around 0.24. Grammatical evolution models maintain exceptionally
low and stable errors, ranging between 0.16-0.17 regardless of distance.
Distance (Dc) significantly affects errors in non-grammatical evolution
models, with particularly large variations in MLP(BFGS). In contrast,
grammatical evolution models maintain nearly constant errors, demonstrating
resilience to spatial variations. Overall, the results support the
superiority of grammatical evolution models, which show smaller and
more stable prediction errors, making them more reliable for earthquake
...
Figure \ref{fig:2010} shows regression errors for 2010 earthquake
magnitude prediction. As in 2004, grammatical evolution models (RULE,
NNC, FC) maintain lower and more stable errors compared to non-grammatical
evolution models (MLP(BP), MLP(RPROP), MLP(BFGS)). MLP(BFGS) shows
significant improvement from 2004, with errors between 0.4-0.74, though
still displaying the highest distance-dependent variation. MLP(RPROP)
maintains the lowest errors among non-grammatical models (0.21-0.32),
while MLP(BP) shows similar errors to 2004 (0.31-0.4). Grammatical
evolution models show slightly higher errors than 2004 but remain
stable and reliable. NNC continues to perform best (0.17-0.18), followed
by FC and RULE (0.17-0.19). Distance has less impact than in 2004,
particularly for MLP(BFGS) which shows reduced variation. Grammatical
evolution models remain largely unaffected by distance. The results
confirm the superiority of grammatical evolution models, which maintain
smaller, more stable errors, while non-grammatical models show improvement
(especially MLP(BFGS)) without matching the former's accuracy.
...
\end{figure}
 Figure \ref{fig:2012} reveals interesting regression error patterns
for 2012. Grammatical evolution models (RULE, NNC, FC) maintain previous
years' trends with exceptionally stable, low errors (0.16-0.19). NNC
and FC perform best (0.16-0.17 for most distances). Non-grammatical
models show mixed results: MLP(BFGS) exhibits extreme variations (0.21-0.85),
demonstrating high condition sensitivity; MLP(RPROP) maintains this
category's lowest errors (0.21-0.29); while MLP(BP) shows more stable
behavior (0.33-0.38). Distance remains significant for non-grammatical
models, especially MLP(BFGS) with 0.64 variation (0.21-0.85). Grammatical
evolution models maintain practically constant errors regardless of
distance. The 2012 data confirms the consistent superiority of grammatical
evolution models, which maintain reliable performance across all distances.
While non-grammatical models show some improvements, their significant
variability continues to limit reliability.
